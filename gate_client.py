...
class GateClient:
    ...
    def get_historical_klines(self, symbol: str, interval: str,
        ...
        # 调整时间范围，确保不超过API限制
        # Gate.io API可能对时间范围有更严格的限制
        max_time_range = interval_seconds * 1000  # 限制为1000个周期
        
        while current_start <= final_end:
            request_end = min(
                final_end,
                current_start + max_time_range
            )
            # 确保时间范围合理，避免请求过多数据
            if request_end - current_start > interval_seconds * 1000:
                request_end = current_start + interval_seconds * 1000
