...
class GateClient:
    ...
    def _extract_base_symbol(self, symbol: str) -> str:
        symbol_upper = symbol.upper()
        if "_" in symbol_upper:
            return symbol_upper.split("_", 1)[0]
        if symbol_upper.endswith("USDT") and len(symbol_upper) > 4:
        ...
        try:
            ...
            for item in data:
                ...
                if quote_asset.upper() not in name.upper():
                    continue
                symbols.append(self._to_internal_symbol(name))
    ...
    def get_hot_symbols_by_volume(self, quote_asset: str = "USDT", top_n: int = 50) -> List[str]:
        try:
            ...
            for item in data:
                ...
                try:
                    hot_symbols.append({'symbol': self._to_internal_symbol(symbol), 'volume': float(volume)})
    ...
    def get_latest_price(self, symbol: str) -> float:
        ...
        if isinstance(data, list):
            for item in data:
                ...
                if contract and self._normalize_symbol(contract) == normalized_symbol:
                    ticker = item
                    break
            if ticker is None and data:
                ticker = data[0]
        else:
            ticker = data
                ...
                while not stop_event.is_set():
                    try:
                        ...
                        if last_open_time is None or open_time > last_open_time:
                            ...
                            kline_data = {
                                'symbol': self._to_internal_symbol(normalized_symbol),
                                'interval': interval,
